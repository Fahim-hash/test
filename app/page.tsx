...
function starterFromDeck(deck:Card[]){
 const idx=deck.findIndex(c=>c.kind==='number');
 return idx>=0 ? deck.splice(idx,1)[0] : {id:uid(),color:'crimson' as Color,value:1,dir:'up' as const,kind:'number' as const,label:'1'};
}
function canPlay(card:Card, top?:Card){
 ...
 if(value===undefined||topValue===undefined) return false;
 return top.dir==='any'||card.dir==='any'||value===topValue||(top.dir==='up'&&value>topValue)||(top.dir==='down'&&value<topValue);
}
...
function initialGame(name:string):Game{
 const deck=makeDeck();
 const hand=deck.splice(0,5);
 const first=starterFromDeck(deck);
 return {players:[{id:uid(),name,hand,connected:true}],deck,chains:[[first]],activeHead:0,turn:0,turnDir:1,winner:null,locked:false,log:['Starter Link established.']};
}

export default function Home(){
 const [screen,setScreen]=useState<'home'|'lobby'|'game'>('home'); const [name,setName]=useState('PLAYER 01'); const [room,setRoom]=useState(''); const [game,setGame]=useState<Game|null>(null); const [selected,setSelected]=useState<string|null>(null); const [online,setOnline]=useState(false); const channel=useRef<any>(null); const myId=useRef(uid()); const host=useRef(false); const gameRef=useRef<Game|null>(null);
 const setGameSafe=(g:Game|null)=>{gameRef.current=g;setGame(g)};
 ...
 async function create(){const code=Math.random().toString(36).slice(2,8).toUpperCase();setRoom(code);host.current=true;const g=initialGame(name);g.players[0].id=myId.current;setGameSafe(g);setScreen('lobby');const ch=supabase.channel(`kinetix:${code}`,{config:{broadcast:{self:false}}});channel.current=ch;ch.on('broadcast',{event:'join'},({payload}:any)=>{const current=gameRef.current;if(!current||current.players.some(p=>p.id===payload.id))return;const hand=current.deck.splice(0,5);current.players.push({id:payload.id,name:payload.name,hand,connected:true});setGameSafe({...current});broadcast(current)}).on('broadcast',{event:'intent'},({payload}:any)=>{if(host.current) handleIntent(payload)}).subscribe(()=>setOnline(true));}
 async function join(){if(!room.trim())return;host.current=false;const id=myId.current;const ch=supabase.channel(`kinetix:${room.trim().toUpperCase()}`,{config:{broadcast:{self:true}}});channel.current=ch;if(ch){ch.on('broadcast',{event:'state'},({payload}:any)=>{setGameSafe(payload);setScreen('game')}).subscribe(async status=>{if(status==='SUBSCRIBED'){setOnline(true);await ch.send({type:'broadcast',event:'join',payload:{id,name}})}})} }
 function start(){if(!game)return;setScreen('game');broadcast(game)}
 function handleIntent(x:any){const current=gameRef.current;if(!current)return;const g=structuredClone(current);const p=g.players[g.turn];if(!p||p.id!==x.player)return;if(x.action==='draw'){const c=g.deck.shift();if(c)p.hand.push(c);g.log.unshift(`${p.name} drew a card.`)}
 ...
 setGameSafe(g);broadcast(g)}
 function applyCard(g:Game,c:Card){const p=g.players[g.turn];const activeChain=g.chains[g.activeHead]??(g.chains[g.activeHead]=[]);activeChain.push(c);g.log.unshift(`${p.name} played ${c.kind==='number'?`#${c.value}`:c.label}.`);
   ...
   if(c.kind==='purge'){g.chains=[[starterFromDeck(g.deck)]];g.activeHead=0;g.log.unshift('PURGE: all active chains cleared.');}
 ...
 function act(c?:Card){if(!game||!me||game.winner||me.id!==active?.id)return;if(c){if(!playable.has(c.id))return;if(selected!==c.id){setSelected(c.id);return}handleIntent({player:me.id,action:'play',id:c.id});setSelected(null)}else handleIntent({player:me.id,action:'draw'})}
 useEffect(()=>()=>{channel.current?.unsubscribe?.()},[]);
 const copy=()=>navigator.clipboard?.writeText(room);
 return <main className="kx-app"><header><div className="kx-brand"><div className="kx-mark">K</div><div><b>KINETIX</b><small>ONLINE EDITION / 02</small></div></div><div className="connection"><i className={online?'live':''}/>{online?'REALTIME LINK':'LOCAL PRACTICE'}</div><button className="icon-btn" onClick={()=>{channel.current?.unsubscribe?.();setScreen('home');setGameSafe(null)}}><LogOut size={15}/> EXIT</button></header>
